...
function App() {
  ...
  const checkGeofenceByDestination = () => {
    if (!geoDestination) {
      ...
      {geoOpen && (
        <div className="modal-overlay" onClick={closeGeoModal}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="geo-description">Check safety status by destination or by your current location.</p>
              ...
              <div className={`geo-result-card ${geoResult.live ? "geo-neutral" : (geoResult.locationOff ? "geo-neutral" : (geoResult.inside_safe_zone ? "geo-safe" : "geo-danger"))}`}>
                {geoResult.live ? (
                  ...
                  {geoResult && (() => {
                    const destLat = geoResult.live ? geoUserPos?.lat : geoResult.dest_lat
