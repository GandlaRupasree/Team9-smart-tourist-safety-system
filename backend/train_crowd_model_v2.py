...
if os.path.exists("crowd_model.pkl"):
    shutil.copy("crowd_model.pkl", "crowd_model_backup2.pkl")
    print("Backed up current model to crowd_model_backup2.pkl")
...
for dest in destinations:
    ...
    lodging_count = db.real_hotels.count_documents({"destination_name": dest["name"]})

    review_popularity = (review_count / 3000) * 0.7 + (rating / 5) * 0.3
    lodging_popularity = min(1.0, lodging_count / 20)
    popularity = (review_popularity * 0.6) + (lodging_popularity * 0.4)
    ...
    for _ in range(120):
        ...
        rows.append({
            ...
            "entry_fee": entry_fee, "lodging_count": lodging_count, "visitors": val,
        })
